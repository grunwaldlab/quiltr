...
file_path_a <- file.path(folder_path_a, "file_a.txt")
file_path_b <- file.path(folder_path_b, "file_b.txt")
file.create(file_path_a)
file.create(file_path_b)
...
#|
#| ### Test that files are found
output <- quiltr:::get_target_paths(root_path, search_type = c("root", "children"))
test_that("files are found", {
  expect_true(file_path_a %in% output)
  expect_true(file_path_b %in% output)
})

test_that("folders are found", {
  expect_true(folder_path_a %in% output)
  expect_true(folder_path_b %in% output)
})
