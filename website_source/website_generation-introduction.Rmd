---
title: Introduction to `make_website`
output:
  html_document:
    toc: true
...
* A menu hierarchy to organize content on the website is inferred from HTML file paths and/or configuration files dependeding on options used. 
* Each page of the website (i.e. each location in the menu) is then made. They consists of a hierarchical dropdown menu linking to all other pages and a series of iframes used to display HTML files assigned to that page. The iframes are automatically scaled to fit their content.
...
quiltr::make_website(path = "vignette_examples/test_website_1", output = "inst/doc/website_source", overwrite = TRUE, output_dir_name = "test_website_1", theme = "cerulean")
```
...
# This is not run automatically due to rmarkdown issue #248, but is used to make the website example
quiltr::make_website(path = "vignette_examples/test_website_2", output = "inst/doc/website_source", overwrite = TRUE, output_dir_name = "test_website_2", note_config_name = "note_config.yml", theme = "cerulean")
```
...
Option values specified replace the defaults when running `make_website`, although options specified in the configuration file can still be overwritten by function options used. 
Unlike the content placement comfiguration files, only one of this type of this type can be used each time a website is built. 
They can be used to store the options used to build a specific website in the source directory.
