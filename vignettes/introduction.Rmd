...
A single function called `quilt` is given a target directory containing HTML files to combine into a website and a location to output a website directory. Below is a simplified summary of how it works: 
...
Option values specified replace the defaults when running `quilt`, although options specified in the configuration file can still be overwritten by function options used. 
...
The configuration file used can be specified by a `quilt` option, so that an existing website can be rebuilt without having to remember the options used.
...
See the following section for potential `quilt` options used when building the website.
