...
The primary purpose of this package is to provide a way to combine HTML files scattered throughout a file system into a single website with a hierarchical drop down menu.
...
Making simple static websites for other purposes is also possible, but quiltr is not designed be an all-purpose website generator such as Jekyll or Drupal.

All of the content of the webpage, besides the drop down menu and other basic site structure elements, is in the form of HTML files created by the user.
...
It is so flexible that website can be made form any directory containing HTML files, regardless if the HTML files or the directory were made for this purpose.
...
A single function called `make_website` is given a target directory containing HTML files to combine into a website and a location to output a website directory. Below is a simplified summary of how it works: 

* In the website output directory, a copy of the target directory is made, copying only the HTML files and any local files (e.g. images) they reference (via HTML tag attributes like `src` and `href`). 
* A menu hierarchy to organize content on the website is inferred from HTML file paths and/or configuration files depending on options used. 
...
Together, the partial copy of the target directory and the pages used to display HTML files therein, constitute a portable and self-contained website.
...
The placement of HTML content can be determined by the names of directories in its path relative to the target directory.
...
Option values specified in this file replace the defaults when running `make_website`, they can still be overwritten by options specified in the function call. 
There can be one website build configuration file per website build, typically in the root of target directory although any configuration file can be specified using the `site_config_file` option. 
By default, a website build configuration file in the target directory will be used if it exists. 
The configuration file can be used to rebuild the website without having to remember the options used every time.
In addition to the configuration file in the target directory, every time a website is made, a configuration file containing the option values used in that instance is created in the output directory. 
By referencing this with the `site_config_file` option of `make_website`, the website can be rebuilt using the same options as last time. 
Below is an example of configuration file content:
